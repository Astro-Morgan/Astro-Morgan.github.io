...
My research aims to build these tools.
...
1) AION Embeddings: High-level, 768-dimensional sequence embeddings from the Polymathic-AI AION-base foundation model (Parker et al. 2025).

2) Denoised Spectra: 1D spectral data, pre-processed using a whitened-Gaussian smoothing kernel and passed through a 1D CNN encoder.
...
1) 'Aristotle' is a lightweight (48M-parameter) "student" model, distilled from 'Plato'. It functions as a fast and efficient embedding tool.

2) 'Kanon Flow' is a redshift-conditioned Continuous Normalizing Flow (CNF) trained on 'Aristotle's latent space.
...
1) A drift $f(\phi,z)$ (the 'average' evolutionary path)

2) A source/sink term $g(\phi,z)$ (population birth/death)
